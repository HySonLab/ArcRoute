...
if __name__ == "__main__":
    ...
    dataloader = DataLoader(
        CARPGenerator(100, 20, 20, 3, "../data/temp_data.pt"),
        batch_size=10,
        shuffle=False,
        num_workers=2,
        collate_fn=CARPGenerator.collate_fn
    )

    for td in tqdm(dataloader):
        # td = td.expand(3, *td.shape)
        # td = torch.stack([*td]*3)
        # td = torch.cat([td] * 3, dim=0)
        # print(td[10]['clss'])
        idxs = torch.cat([torch.randperm(td.size(0)) for _ in range(3)])
        # print(idxs)
        # td = td[idxs]
        # exit()
        for i in range(0, td.size(0)*3, 32):
            id_sub = idxs[i:i+32]
            td_sub = td[id_sub]
            print(id_sub.shape)
